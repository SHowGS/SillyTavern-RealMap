...
- 全屏内提供“禁用”和“重新判断”按钮；移动端点击“禁用”会先退出全屏，再显示禁用确认框。
- 公交详情以底部面板显示，路线策略切换控件会适配面板高度。

### 聊天状态与回档

- 启用状态按聊天保存在`chat_metadata.realmap_enabled`，明确禁用后会长期保持，刷新或重新进入聊天不会再次询问；只有从未选择过启用状态的聊天才会询问。禁用会立即取消本聊天正在进行的插件LLM调用、停止等待并丢弃地图回调、清除两类系统提示注入，同时关闭小地图和全屏地图；后续发送、重新生成、切换回复及群聊生成均不会恢复旧注入或写入地图数据。
...
前置路线信息只在当次主回复生成期间临时注入，不写入聊天消息，也不会直接修改当前位置：
...
- 正文输出后不会自动显示调试弹窗。最近一轮正文前信息补充LLM和输出后位置推断LLM的请求消息、65536tokens输出上限、HTTP状态、请求ID、响应模型、`finish_reason`、`usage`、完整API响应、LLM原始输出、JSON解析结果和前置地图注入结果会缓存在当前浏览器标签页；点击配置页“日志”按钮后统一显示。正文前信息补充LLM不读取或写入聊天路线缓存，正常发送、重新生成和切换回复都会重新调用。关闭标签页后日志自动清除。
